        ...
        children: [
              ...
              Expanded(
                child: Card(
                  child: ListTile(
                    ...
                    subtitle: Text(memory == null ? '—' : '${memory['usedPercent']}% used'),
                  ),
                ),
              ),
            ],
          ...
          Card(
            child: ListTile(
              ...
              subtitle: Text(docker == null ? 'No live inventory.' : '${docker['running']} / ${docker['total']} running'),
            ),
          ),
          Card(
            child: ListTile(
              leading: Icon(stopped.isEmpty ? Icons.check_circle_outline : Icons.warning_amber_outlined),
              title: Text(stopped.isEmpty ? 'No stopped containers' : '${stopped.length} stopped container${stopped.length == 1 ? '' : 's'}'),
              subtitle: Text(stopped.isEmpty ? 'All monitored workloads are running.' : stopped.join(', ')),
            ),
          ),
          const SizedBox(height: 18),
          if (admin)
            const Card(
              child: Padding(
                ...
                child: Text('Admin safety: restart actions will be shown here only after a server-side confirmation endpoint is configured. This app never restarts containers automatically.'),
              ),
            ),
          FilledButton.icon(
            onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const HomelabWebAppScreen(title: 'Unraid', url: 'https://home.conhomelab.uk'))),
